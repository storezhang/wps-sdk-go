...
type Reader struct {
	...
	// 默认值是0.6
	MarkTransparent float32 `json:"markTransparent"`
	// 字体CSS样式，比如bold 20px Serif
	MarkFontCssStyle string `json:"markFontCssStyle"`
	// 水印旋转度，可取值是1到360
	MarkRotate int `json:"markRotate"`
	// 水印水平距离，可取值是1到100
	MarkHorizontal int8 `json:"markHorizontal"`
	// 水印垂直距离，可取值是1到100
	MarkVertical int8 `json:"markVertical"`
	// 是否可打印，默认false
	...
	// 第6位：默认1（显示），是否显示格式修订
	WpsPreview string `json:"wpsPreview"`
}
